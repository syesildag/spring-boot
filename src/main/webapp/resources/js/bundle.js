...
(function (JReactComponents) {
    ...
    var List = (function (_super) {
        ...
        List.prototype.updateVariableFrame = function (eventObject) {
        ...
        List.prototype.cacheSizes = function () {
            ...
            var itemEls = this.items.element[0].children;
            var sizeKey = getOffsetSizeKey(this.props.axis);
            for (var i = 0, l = itemEls.length; i < l; ++i) {
                cache[fromIndex + i] = itemEls[i][sizeKey];
            }
        };
        List.prototype.updateUniformFrame = function (eventObject) {
        ...
        List.prototype.getItemSizeAndItemsPerRow = function () {
            ...
            var itemEls = JReact.getInstance(JReact.getInstance(this.refs[ITEMS_CONTAINER]).refs[ITEMS_TRANSLATE]).refs[ITEMS_REFERENCE][0].children;
            if (!itemEls.length)
                return {};
        ...
        List.prototype.getSizeOf = function (index) {
            var _a = this, cache = _a.cache, items = _a.items;
            var _b = this.props, axis = _b.axis, itemSizeGetter = _b.itemSizeGetter, itemSizeEstimator = _b.itemSizeEstimator, listType = _b.listType;
            var _c = this.state, fromIndex = _c.fromIndex, itemSize = _c.itemSize, size = _c.size;
            // Try the static itemSize.
            if (itemSize)
                return itemSize;
